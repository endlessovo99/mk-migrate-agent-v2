...
The separately confirmed locked-draft recovery Interface implements that exception without widening normal execution. It binds the complete historical DSL and prior report by approved SHA-256 digests, rebuilds the current Source Draft, permits only an explicitly enumerated readback-only catalog-version bridge, resolves identities from retained successful execution evidence plus current target lookup, and requires two identical native snapshots. Record-only reconciliation performs no template or workflow write. Scoped repair patches the current native snapshot rather than reprojecting the complete template and permits only the evidenced authorization collections or the three calculation-native paths. A deterministic per-origin/target exclusive lock, stored independently of the caller's artifacts directory and mirrored into it, precedes the first write; success, failure, or outcome uncertainty all prevent automatic retry. Complete independent readback remains required before the transfer-record callback.
