...
Title patches require `confidence >= 0.7`. Type, component, and props patches require `confidence >= 0.85`. Props patches are limited to `required`, `options`, `maxLength`, and `placeholder` and must be supported by source evidence. A placeholder-only onLoad may close as static-property coverage only when the Translator has already materialized the exact literal on a target component that supports `placeholder`, and coverage records that same value.
