...
- Locked-draft recovery is a separate, explicitly confirmed exception rather than a mode of normal execution. It requires one retained `readback_failed` creation report, complete approved artifact digests, the exact target/category/origin, a stable two-read native snapshot, private backups, a fresh artifacts directory, and a persistent single-attempt state file.
