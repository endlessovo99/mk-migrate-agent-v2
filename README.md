...
An existing draft whose original creation stopped at `readback_failed` before any transfer-record attempt can use the narrow `recover-transfer-record` command. Recovery requires the exact prior execution report, the same trusted DSL digests and NewOA origin, the original target/category IDs, a fixed record ID, `--confirm-write`, and `--confirm-no-successful-transfer-record`. It performs only login, participant/condition lookup, target/workflow readback, full invariant and number-rule verification, and one transfer-record callback. It never updates the template or workflow, refuses any prior callback or outcome-unknown evidence, and never retries an uncertain callback.
