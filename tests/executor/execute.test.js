...
describe("executeDsl", () => {
  it("writes one draft template through an injected NewOA client and verifies readback", async () => {
    ...
    assert.deepEqual(result.apiStages.map((stage) => stage.name), ["login", "add", "get", "update", "readback"]);
    assert.deepEqual(client.calls.map((call) => call.name), ["login", "addTemplate", "getTemplate", "updateTemplate", "getTemplate"]);
    ...
    assert.equal(xformConfig.migrationDsl.form.fieldCount, 2);
    ...
    assert.equal(result.readback.form.fieldCount, 2);
  ...
  it("fails readback when persisted designer structure loses layout cells", async () => {
    const client = new FakeNewoaClient({
      corruptReadback(template) {
        ...
        sceneConfig.view.render.desktop.children[1].cells = [];
  ...
  it("reports warnings when unknown workflow nodes are mapped conservatively", async () => {
    ...
    assert.equal(result.diagnostics.some((diagnostic) => diagnostic.code === "workflow.node_type_mapped_to_manual_task"), true);
  });
